...
const DashBoard = () => {
  ...
  const fetchTasks = async () => {
    try {
      const res = await axios.get("http://localhost:4000/app/task/test@gmail.com");
    ...
    <div className="w-full min-h-screen bg-[#0f172a] text-white px-6 py-8">
      ...
      <div className="border border-gray-700 rounded-xl bg-[#1e293b] p-6 flex flex-col gap-4 shadow-lg h-[85vh] overflow-y-auto scrollbar-thin scrollbar-thumb-[#475569] scrollbar-track-[#1e293b]">
        ...
        <div className="flex flex-col gap-4">
          {toggle === "Completed" &&
            (tasks.filter((item: any) => item.status === "completed").length > 0 ? (
              tasks
          ...
          {toggle === "pending" &&
            (tasks.filter((item: any) => item.status === "pending").length > 0 ? (
              tasks
                .filter((item: any) => item.status === "pending")
                .map((item: any, index: number) => <PendingTask item={item} key={index} />)
            ) : (
              <div className="w-full flex justify-center items-center text-gray-400 text-lg h-full">
          ...
          {toggle === "in-progress" &&
            (tasks.filter((item: any) => item.status === "in-progress").length > 0 ? (
              tasks
